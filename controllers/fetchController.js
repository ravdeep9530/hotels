...
module.exports.getLastTenBookingTransactions=function(req,res){
    ...
    connection.query("CALL `C_lastTenBookingTransactions`();", function (error, results, fields) {
        {
{
                res.json({status:'true',
                    data: results
...
module.exports.getReports=function(req,res){
    ...
    connection.query('CALL `C_Reports`('+req.params.id+',CAST(NOW() AS DATE));', function (error, result1, fields) {
        {
            console.log(result1)
res.json({data: result1,status:'true',error:error
